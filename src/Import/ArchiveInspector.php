...
final class ArchiveInspector
{
    ...
    /**
     * @return array{source_url: string, home_url: string, source_prefix: string}
     */
    public function inspect(string $archivePath): array
    {
        $result = ['source_url' => '', 'home_url' => '', 'source_prefix' => ''];
        ...
        if ($dumpEntry !== null) {
            ...
            if ($result['source_prefix'] === '') {
                $result['source_prefix'] = $this->scanDumpForPrefix($archivePath, $offset, $header->size());
            }
        }
    ...
    /**
     * Read first CREATE TABLE statement and extract its table name prefix.
     * Returns '' if nothing found.
     */
    private function scanDumpForPrefix(string $archivePath, int $offset, int $size): string
    {
        if ($size <= 0) return '';
        ...
        try {
            ...
            $budget = (int) min(256 * 1024, $size); // first 256 KB is more than enough to find CREATE TABLE
            $chunk = fread($handle, $budget);
            if ($chunk === false) return '';
            // Match: CREATE TABLE `prefix_tablename` ...
            // or:    CREATE TABLE IF NOT EXISTS `prefix_tablename`
            if (preg_match('/CREATE\s+TABLE(?:\s+IF\s+NOT\s+EXISTS)?\s+`([^`]+)`/i', $chunk, $m) === 1) {
                $full = $m[1];
                // Take everything up to the LAST underscore+name piece; WP tables are prefix_x_y.
                // Best heuristic: find the first underscore. That gives us `prefix_`.
                $underscore = strpos($full, '_');
                if ($underscore !== false) {
                    return substr($full, 0, $underscore + 1);
                }
            }
            return '';
        } finally {
    ...
    private function readMetadata(string $archivePath, int $offset, int $size, array &$result): void
    {
        ...
        try {
            ...
            if ($result['source_prefix'] === '' && isset($data['source_prefix']) && is_string($data['source_prefix'])) {
